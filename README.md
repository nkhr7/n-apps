...
`apps/youtube-schedule/` は、登録したYouTubeチャンネルの配信中・配信予定を定期的に確認して表示するページです。サーバーを持たない静的サイトのため、閲覧者自身のYouTube Data API v3キーとチャンネルURL（`/channel/UC...`・`/@ハンドル`・`/user/ユーザー名` に対応、`/c/カスタムURL` は検索APIでの代替解決になるためクォータ消費が大きく非推奨）をブラウザに保存して使う方式にしています。

APIクォータを節約するため、`search.list`（100ユニット）ではなく `channels.list` → `playlistItems.list` → `videos.list`（あわせて数ユニット）でアップロード一覧から配信中・配信予定を判定しています。チャンネルIDの解決結果はキャッシュし、チャンネルURLを変更しない限り再解決しません。取得に失敗した場合は、前回取得できたデータをキャッシュから表示しつつエラー内容を案内します。
